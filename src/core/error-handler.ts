 ...
 */
class DefaultErrorContext implements ErrorContext {
  getErrorMessage(error: Error, context: string): string {
...
let globalErrorContext: ErrorContext = new DefaultErrorContext();

/**
 * Set the global error context for dependency injection
 * @param errorContext - Implementation-specific error context
 */
export function setErrorContext(errorContext: ErrorContext): void {
...
export function handleError(
  ...
  const {
    ...
    skipError = false,
    showStack = false,
    errorContext = globalErrorContext
  } = options;
 ...
 */
function provideContextualGuidance(
  category: ErrorCategory, 
  exitCode: number, 
  error: Error, 
  context: string,
