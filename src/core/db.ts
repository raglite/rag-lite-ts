...
export interface ContentChunk {
  ...
  embedding_id: string;
  document_id: number;
  content: string;           // Generic content (text, image path, etc.)
...
export async function insertChunk(
  ...
  try {
    ...
    await connection.run(
      'INSERT OR REPLACE INTO chunks (embedding_id, document_id, content, chunk_index, content_type, metadata) VALUES (?, ?, ?, ?, ?, ?)',
      [embeddingId, documentId, content, chunkIndex, contentType, metadataJson]
    );
  } catch (error) {
 ...
 */
export async function hasDatabaseData(connection: DatabaseConnection): Promise<boolean> {
  try {
