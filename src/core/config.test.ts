...
import { CoreConfig, validateCoreConfig, getModelDefaults, type ModelDefaults, ConfigurationError } from './config.js';
...
describe('Configuration Tests', () => {
  describe('getModelDefaults', () => {
    ...
    test('should handle case sensitivity correctly', () => {
      // Test case variations - should match case-insensitively
      const testCases = [
        {
          input: 'SENTENCE-TRANSFORMERS/ALL-MINILM-L6-V2',
          expected: { dimensions: 384, chunk_size: 250, chunk_overlap: 50, batch_size: 16 }
        },
        {
          input: 'Sentence-Transformers/All-MiniLM-L6-v2',
          expected: { dimensions: 384, chunk_size: 250, chunk_overlap: 50, batch_size: 16 }
        ...
        {
          input: 'XENOVA/ALL-MPNET-BASE-V2',
          expected: { dimensions: 768, chunk_size: 400, chunk_overlap: 80, batch_size: 8 }
        },
        {
          input: 'xenova/all-mpnet-base-v2',
          expected: { dimensions: 768, chunk_size: 400, chunk_overlap: 80, batch_size: 8 }
      ...
      for (const testCase of testCases) {
        const defaults = getModelDefaults(testCase.input);
        assert.deepEqual(defaults, testCase.expected,
          `Expected case variation "${testCase.input}" to return correct configuration`);
      }
    });
  });
  ...
  describe('Model Cache Path Configuration', () => {
    ...
    test('should validate model_cache_path field correctly', () => {
      ...
      for (const config of validConfigs) {
        assert.doesNotThrow(() => validateCoreConfig(config),
      ...
      for (const config of invalidConfigs) {
        assert.throws(() => validateCoreConfig(config),
    ...
    test('should include model_cache_path in Config interface', () => {
      ...
      const configWithCachePath: CoreConfig = {
        embedding_model: 'sentence-transformers/all-MiniLM-L6-v2',
        chunk_size: 250,
        chunk_overlap: 50,
        batch_size: 16,
      ...
      const configWithoutCachePath: CoreConfig = {
        embedding_model: 'sentence-transformers/all-MiniLM-L6-v2',
        chunk_size: 250,
        chunk_overlap: 50,
        batch_size: 16,
      ...
      assert.doesNotThrow(() => validateCoreConfig(configWithCachePath),
        'Config with model_cache_path should pass validation');
      assert.doesNotThrow(() => validateCoreConfig(configWithoutCachePath),
