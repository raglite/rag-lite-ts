...
export class ModelRegistry {
  ...
  private static validateModelRequirements(
    ...
    if (modelInfo.requirements.requiredFeatures) {
      ...
      const hasAdvancedFeatures = modelInfo.requirements.requiredFeatures.some(
        feature => advancedFeatures.includes(feature)
      );
  ...
  private static validateModelCapabilities(
    ...
    if (modelInfo.capabilities.supportsImages && modelInfo.capabilities.supportedImageFormats) {
      ...
      const supportedCommon = commonFormats.filter(format =>
        modelInfo.capabilities.supportedImageFormats!.includes(format)
      );
  ...
  static getSupportedModels(modelType?: ModelType): string[] {
    ...
    return allModels.filter(modelName =>
      SUPPORTED_MODELS[modelName].type === modelType
    );
  }
  ...
  static getModelsByContentType(contentType: string): string[] {
    return Object.keys(SUPPORTED_MODELS).filter(modelName =>
      SUPPORTED_MODELS[modelName].supportedContentTypes.includes(contentType)
    );
  }
   ...
   */
  private static getSimilarModels(modelName: string): string[] {
    const allModels = Object.keys(SUPPORTED_MODELS);
 ...
 */
export function getModelType(modelName: string): ModelType | null {
 ...
 */
export function isTextOnlyModel(modelName: string): boolean {
  const modelInfo = ModelRegistry.getModelInfo(modelName);
  return modelInfo ?
 ...
 */
export function getRecommendedBatchSize(modelName: string): number {
  const modelInfo = ModelRegistry.getModelInfo(modelName);
 ...
 */
export const DEFAULT_MODELS: Record<ModelType, string> = {
