...
interface ResourceInfo {
  id: string;
  type: 'embedder' | 'model' | 'processor' | 'cache';
  name: string;
...
export class ResourceManager {
  ...
  private memoryThresholdMB: number = 512; // Default 512MB threshold
   ...
   */
  registerEmbedder(embedder: UniversalEmbedder): string {
   ...
   */
  registerModel(model: any, modelName: string, modelType: string): string {
   ...
   */
  registerImageProcessor(processor: any, processorName: string): string {
   ...
   */
  async cleanupResourcesByType(type: ResourceInfo['type']): Promise<CleanupStats> {
   ...
   */
  async cleanupAllResources(): Promise<CleanupStats> {
    const startTime = Date.now();
   ...
   */
  async cleanupUnusedResources(maxAgeMinutes: number = 30): Promise<CleanupStats> {
    const startTime = Date.now();
    const cutoffTime = new Date(Date.now() - maxAgeMinutes * 60 * 1000);
    
    const unusedResources = Array.from(this.resources.values()).filter(
      resource => resource.lastUsedAt < cutoffTime
    );
   ...
   */
  setGarbageCollection(enabled: boolean): void {
  ...
  getResourceStats(): {
    ...
    oldestResource: { name: string; ageMinutes: number } | null;
    newestResource: { name: string; ageMinutes: number } | null;
   ...
   */
  private estimateEmbedderMemory(embedder: UniversalEmbedder): number {
   ...
   */
  private estimateModelMemory(modelType: string): number {
   ...
   */
  private estimateProcessorMemory(processorName: string): number {
 ...
 */
export function registerEmbedder(embedder: UniversalEmbedder): string {
 ...
 */
export async function cleanupAndOptimizeMemory(): Promise<void> {
