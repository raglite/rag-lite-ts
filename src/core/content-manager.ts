...
export interface ContentManagerConfig {
  contentDir: string;
  maxFileSize: number | string; // in bytes or string like "50MB"
  maxContentDirSize: number | string; // in bytes or string like "2GB"
  enableDeduplication: boolean;
  enableStorageTracking: boolean;
  storageWarningThreshold: number; // percentage (0-100) when to warn about storage usage
  storageErrorThreshold: number; // percentage (0-100) when to reject new content
}

/**
 * Internal configuration with normalized values
 */
interface NormalizedContentManagerConfig {
  contentDir: string;
  maxFileSize: number; // in bytes
  maxContentDirSize: number; // in bytes
  enableDeduplication: boolean;
  enableStorageTracking: boolean;
  storageWarningThreshold: number; // percentage (0-100) when to warn about storage usage
  storageErrorThreshold: number; // percentage (0-100) when to reject new content
 ...
 */
const DEFAULT_CONFIG: NormalizedContentManagerConfig = {
...
export class ContentManager {
  ...
  private config: NormalizedContentManagerConfig;
   ...
   */
  async ingestFromFilesystem(filePath: string): Promise<ContentIngestionResult> {
      ...
      try {
        ...
        if (stats.size > 10 * 1024 * 1024) { // Use streaming for files > 10MB
          contentHash = await withTimeout(
            this.performanceOptimizer.calculateFileHashOptimized(filePath),
            120000, // 2 minute timeout for large file hashing
          ...
          content = await withTimeout(
            ...
            60000, // 60 second timeout for file reading
        ...
        if (this.config.enableDeduplication) {
          const existing = await withTimeout(
            getContentMetadataByHash(this.db, contentHash),
        ...
        await withTimeout(
          insertContentMetadata(this.db, contentMetadata),
          10000, // 10 second timeout for database insertion
      ...
      try {
        ...
        if (content.length > this.config.maxFileSize) {
          ...
          throw new ContentIngestionError(
            ...
            `Content size (${sizeMB}MB) exceeds maximum allowed size (${limitMB}MB)`,
            'memory_ingestion'
          );
        }
        ...
        await withTimeout(
          this.enforceStorageLimits(content.length),
          30000, // 30 second timeout for storage limit checks
        ...
        contentHash = await withTimeout(
          this.performanceOptimizer.calculateBufferHashOptimized(safeBuffer.get()),
          120000, // 2 minute timeout for hash calculation
        ...
        await withTimeout(
          ...
          5000, // 5 second timeout for directory creation
        ...
        if (content.length > 10 * 1024 * 1024) { // Use streaming for content > 10MB
          const writeResult = await withTimeout(
            this.streamingOps.writeBufferStreaming(safeBuffer.get(), contentPath),
            180000, // 3 minute timeout for large content writing
          ...
          await withTimeout(
            writeFileAtomic(contentPath, safeBuffer.get(), transactionId),
            60000, // 60 second timeout for file writing
        ...
        if (this.config.enableStorageTracking) {
          try {
            await withTimeout(
              ...
              15000, // 15 second timeout for stats update
   ...
   */
  async deduplicateContent(contentId: string): Promise<boolean> {
    try {
   ...
   */
  async enforceStorageLimits(contentSize: number): Promise<void> {
    if (!this.config.enableStorageTracking) {
    ...
    try {
      ...
      if (projectedPercent > this.config.storageWarningThreshold && currentPercent <= this.config.storageWarningThreshold) {
        ...
        console.warn(
          `⚠️  Storage Warning: Content directory usage will reach ${Math.round(projectedPercent)}% after adding this content.\n` +
   ...
   */
  async getStorageLimitStatus(): Promise<{
   ...
   */
  private detectContentType(filePath: string, content?: Buffer): string {
   ...
   * @returns MIME type string
   */
  private detectContentTypeFromBuffer(content: Buffer, filename?: string): string {
   ...
   */
  private detectContentTypeByMagicNumbers(content: Buffer): string {
    if (content.length === 0) {
      return 'application/octet-stream';
    }
   ...
   */
  private detectContentTypeByExtension(extension: string): string {
   ...
   */
  private validateContentType(contentType: string): { isSupported: boolean; error?: string } {
   ...
   */
  private isValidUTF8Byte(buffer: Buffer, index: number): boolean {
  ...
  async getStorageStats(): Promise<{
    ...
    overall: {
      ...
      storageEfficiency: number; // Percentage of actual vs potential duplication
    };
    limits: {
   ...
   */
  async getContentDirectoryStats(): Promise<{
    totalFiles: number;
    totalSize: number;
   ...
   */
  async checkStorageLimits(contentSize: number): Promise<boolean> {
    try {
   ...
   */
  async removeOrphanedFiles(): Promise<{
    removedFiles: string[];
    errors: string[];
    freedSpace: number;
  }> {
   ...
   */
  async removeDuplicateContent(): Promise<{
    removedFiles: string[];
    errors: string[];
    freedSpace: number;
  }> {
   ...
   * @returns Promise that resolves to deduplication results
   */
  async deduplicateContentFiles(): Promise<{
    removedFiles: string[];
    errors: string[];
    freedSpace: number;
  }> {
    ...
    try {
      ...
      const hashGroups = new Map<string, ContentMetadata[]>();
   ...
   */
  async ensureContentDirectoryPermissions(): Promise<void> {
    try {
      await this.ensureContentDirectory();
   ...
   */
  async validateAndRepairContentDirectory(): Promise<{
    isValid: boolean;
    issues: string[];
    repaired: string[];
  }> {
   ...
   */
  private async getActualDirectoryStats(): Promise<{ files: number; size: number }> {
    let files = 0;
    let size = 0;
