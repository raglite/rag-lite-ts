...
export interface ExtensibleConfig<T = {}> extends CoreConfig {
  implementation: T;  // For text-specific, multimodal-specific settings
}
 ...
 */
export function getDefaultModelCachePath(): string {
 ...
 */
export function validateCoreConfig(config: any): asserts config is CoreConfig {
  if (!config || typeof config !== 'object') {
    throw new ConfigurationError('Configuration must be an object');
  }

  // Check required string fields
  const requiredStrings: (keyof CoreConfig)[] = ['db_file', 'index_file'];
  ...
  const requiredNumbers: (keyof CoreConfig)[] = ['chunk_size', 'chunk_overlap', 'batch_size', 'top_k'];
 ...
 */
export function validatePreprocessingConfig(config: any): asserts config is any {
  if (!config || typeof config !== 'object') {
 ...
 */
export function mergePreprocessingConfig(config: any): any {
  const modeDefaults = {
  ...
  const defaults = modeDefaults[config.mode as keyof typeof modeDefaults] || modeDefaults.balanced;
