 ...
 */
const DEFAULT_CONFIG: Required<ActionableErrorConfig> = {
...
export function createMissingFileError(
  filePath: string,
  fileType: 'index' | 'database' | 'config' | 'content',
  config: ActionableErrorConfig = {}
): Error {
...
export function createInvalidPathError(
  paths: { name: string; value: string | undefined }[],
  config: ActionableErrorConfig = {}
): Error {
  const cfg = { ...DEFAULT_CONFIG, ...config };
  const messages: string[] = [];
 ...
 */
export function createModelLoadingError(
  modelName: string,
 ...
 */
export function createDimensionMismatchError(
  expected: number,
  actual: number,
  context: string,
  config: ActionableErrorConfig = {}
 ...
 */
export function createModeMismatchError(
  expectedMode: string,
  actualMode: string,
  config: ActionableErrorConfig = {}
): Error {
...
export function createInvalidContentError(
  contentType: string,
  issue: 'empty' | 'invalid_format' | 'too_large' | 'unsupported',
  config: ActionableErrorConfig = {}
): Error {
  const cfg = { ...DEFAULT_CONFIG, ...config };
  const messages: string[] = [];
...
export function createMissingDependencyError(
  ...
  dependencyType: 'function' | 'object' | 'service',
  config: ActionableErrorConfig = {}
): Error {
  const cfg = { ...DEFAULT_CONFIG, ...config };
  const messages: string[] = [];
 ...
 */
export function createFactoryCreationError(
  factoryName: string,
  originalError: string,
  config: ActionableErrorConfig = {}
): Error {
