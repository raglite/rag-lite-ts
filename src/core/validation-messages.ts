...
import type { ModelInfo, ModelType } from './universal-embedder.js';
...
export const ERROR_MESSAGES = {
    MODEL_NOT_FOUND: (modelName: string, suggestions: string[]) => ({
    ...
    TRANSFORMERS_VERSION_INCOMPATIBLE: (modelName: string, required: string, current: string) => ({
    ...
    INSUFFICIENT_MEMORY: (modelName: string, required: number, available: number) => ({
    ...
    PLATFORM_UNSUPPORTED: (modelName: string, currentPlatform: string, supportedPlatforms: string[]) => ({
    ...
    FEATURES_MISSING: (modelName: string, missingFeatures: string[]) => ({
    ...
    CONTENT_TYPE_UNSUPPORTED: (contentType: string, modelName: string, supportedTypes: string[]) => ({
...
export const WARNING_MESSAGES = {
    HIGH_MEMORY_USAGE: (modelName: string, memoryMB: number) => ({
    ...
    LIMITED_BATCH_SIZE: (modelName: string, maxBatchSize: number) => ({
    ...
    EXPERIMENTAL_FEATURES: (modelName: string, features: string[]) => ({
    ...
    PERFORMANCE_IMPACT: (modelName: string, reason: string) => ({
 ...
 */
export function formatErrorMessage(error: ReturnType<typeof ERROR_MESSAGES[keyof typeof ERROR_MESSAGES]>): string {
    const lines: string[] = [];
 ...
 */
export function formatWarningMessage(warning: ReturnType<typeof WARNING_MESSAGES[keyof typeof WARNING_MESSAGES]>): string {
    const lines: string[] = [];
...
export function createValidationErrorMessage(
    modelName: string,
    errorType: 'not_found' | 'version_incompatible' | 'insufficient_memory' | 'platform_unsupported' | 'features_missing' | 'content_type_unsupported',
    ...
    switch (errorType) {
        ...
        case 'version_incompatible':
            return formatErrorMessage(ERROR_MESSAGES.TRANSFORMERS_VERSION_INCOMPATIBLE(
                modelName,
        ...
        case 'insufficient_memory':
            return formatErrorMessage(ERROR_MESSAGES.INSUFFICIENT_MEMORY(
                modelName,
                context.required || 0,
                context.available || 0
            ));
        ...
        case 'platform_unsupported':
            return formatErrorMessage(ERROR_MESSAGES.PLATFORM_UNSUPPORTED(
                modelName,
                context.currentPlatform || 'unknown',
                context.supportedPlatforms || []
            ));

        case 'features_missing':
            return formatErrorMessage(ERROR_MESSAGES.FEATURES_MISSING(
                modelName,
                context.missingFeatures || []
            ));

        case 'content_type_unsupported':
            return formatErrorMessage(ERROR_MESSAGES.CONTENT_TYPE_UNSUPPORTED(
                context.contentType || 'unknown',
                modelName,
                context.supportedTypes || []
            ));
 ...
 */
export function createModelSuggestions(
    modelType?: ModelType,
    contentTypes?: string[],
    memoryLimit?: number
): string[] {
 ...
 */
export function getTroubleshootingSteps(issue: string): string[] {
