...
import type { ModeType, ModelType } from '../types.js';
 ...
 */
const MODE_MODEL_COMPATIBILITY: Record<ModeType, ModelType[]> = {
 ...
 */
const RECOMMENDED_MODELS: Record<ModeType, string[]> = {
  'text': [
 ...
 * ```
 */
export function validateModeModelCompatibility(
  mode: ModeType,
  modelName: string
): ModeModelValidationResult {
  const errors: string[] = [];
  ...
  if (!compatibleModelTypes.includes(modelInfo.type)) {
    errors.push(
      `Model '${modelName}' (type: ${modelInfo.type}) is not compatible with ${mode} mode`
    );
    
    // Provide specific guidance based on the incompatibility
    if (mode === 'multimodal' && modelInfo.type === 'sentence-transformer') {
      suggestions.push(
        `Multimodal mode requires CLIP models for unified text-image embedding space. ` +
      ...
      warnings.push(
        `Using CLIP model '${modelName}' in text mode. Images will be converted to text descriptions. ` +
  ...
  const unsupportedTypes = requiredContentTypes.filter(
    type => !modelInfo.supportedContentTypes.includes(type)
  );

  if (unsupportedTypes.length > 0) {
    errors.push(
      `Model '${modelName}' does not support required content types for ${mode} mode: ${unsupportedTypes.join(', ')}`
    );
 ...
 * ```
 */
export function validateModeModelCompatibilityOrThrow(
  mode: ModeType,
  modelName: string
): void {
 ...
 * ```
 */
export function getRecommendedModelsForMode(mode: ModeType): string[] {
 ...
 * ```
 */
export function isModeModelCompatible(mode: ModeType, modelName: string): boolean {
  const result = validateModeModelCompatibility(mode, modelName);
 ...
 * ```
 */
export function getCompatibleModelsForMode(mode: ModeType): string[] {
  const compatibleModelTypes = MODE_MODEL_COMPATIBILITY[mode];
