...
// =============================================================================
// TRANSFORMERS.JS COMPATIBILITY SYSTEM
// =============================================================================
 ...
 */
export const TRANSFORMERS_COMPATIBILITY_MATRIX: Record<string, TransformersJsInfo> = {
  '2.6.0': {
    version: '2.6.0',
...
export class ModelValidator {
  private static currentTransformersVersion: string | null = null;
   ...
   */
  static async detectTransformersVersion(): Promise<string | null> {
    try {
   ...
   */
  static async validateModelDetailed(
    modelName: string,
   ...
   */
  static validateTransformersCompatibility(
    modelInfo: ModelInfo,
  ...
  static getRecommendedModels(
    ...
    return compatibleModels.filter(modelName => {
      ...
      const supportsAllTypes = contentTypes.every(type => 
        modelInfo.supportedContentTypes.includes(type)
      );
      
      if (!supportsAllTypes) return false;
      
      // Check memory requirements
      if (maxMemory && modelInfo.requirements.minimumMemory) {
        if (modelInfo.requirements.minimumMemory > maxMemory) return false;
      }
   ...
   */
  private static async checkSystemCompatibility(
    modelInfo: ModelInfo,
    ...
    const features = capabilities?.supportedFeatures && modelInfo.requirements.requiredFeatures ?
      modelInfo.requirements.requiredFeatures.every(feature => 
        capabilities.supportedFeatures!.includes(feature)
      ) :
      true; // Assume compatible if not specified
  ...
  private static generateRecommendations(
    ...
    if (!systemCompatibility.transformersJs || !systemCompatibility.memory) {
      ...
      const sameTypeModels = compatibleModels.filter(name => 
        SUPPORTED_MODELS[name].type === modelInfo.type
      );
   ...
   */
  private static checkVersionCompatibility(currentVersion: string, requiredVersion: string): boolean {
   ...
   */
  private static getCompatibilityInfo(version: string): TransformersJsInfo | null {
 ...
 */
export class EnhancedModelValidationError extends Error implements ModelValidationError {
  public readonly name = 'ModelValidationError';
  
  constructor(
    public readonly modelName: string,
    public readonly availableModels: readonly string[],
    message: string,
    public readonly errorType: 'not_found' | 'incompatible' | 'unsupported_features' = 'not_found',
    public readonly suggestions: readonly string[] = [],
    public readonly troubleshootingSteps: readonly string[] = []
  ) {
    super(message);
  }
 ...
 */
export class EnhancedTransformersCompatibilityError extends Error implements TransformersCompatibilityError {
  public readonly name = 'TransformersCompatibilityError';
  ...
  constructor(
    ...
    message: string,
    public readonly upgradeInstructions: readonly string[] = [],
    public readonly alternativeModels: readonly string[] = []
  ) {
    super(message);
  }
...
export function createModelValidationError(
  modelName: string, 
  reason: string,
  errorType: 'not_found' | 'incompatible' | 'unsupported_features' = 'not_found'
): EnhancedModelValidationError {
  ...
  return new EnhancedModelValidationError(
    modelName,
    availableModels,
    message,
    errorType,
 ...
 */
export function createTransformersCompatibilityError(
  modelName: string,
  requiredVersion: string,
  currentVersion: string
): EnhancedTransformersCompatibilityError {
  ...
  return new EnhancedTransformersCompatibilityError(
    modelName,
    requiredVersion,
    currentVersion,
    message,
 ...
 */
export async function initializeModelValidator(): Promise<boolean> {
  try {
 ...
 */
function generateModelSuggestions(modelName: string, errorType: string): string[] {
  const suggestions: string[] = [];
  const lowerModelName = modelName.toLowerCase();
 ...
 */
function generateTroubleshootingSteps(errorType: string): string[] {
  const steps: string[] = [];
 ...
 */
function generateUpgradeInstructions(requiredVersion: string, currentVersion: string): string[] {
 ...
 */
export async function validateModelWithDetailedErrors(
  modelName: string,
  options: {
  ...
  try {
    ...
    if (checkTransformersVersion) {
      ...
      if (currentVersion) {
        ...
        if (!compatibilityResult.isValid) {
          const error = createTransformersCompatibilityError(
            modelName,
            modelInfo.requirements.transformersJsVersion,
            currentVersion
          );
          if (throwOnError) {
            throw error;
    ...
    const wrappedError = createModelValidationError(
      modelName,
