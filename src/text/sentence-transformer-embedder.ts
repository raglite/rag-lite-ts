...
export class SentenceTransformerEmbedder extends BaseUniversalEmbedder {
  ...
  private engineResourceId?: string;
    ...
    try {
      ...
      if (!this.embeddingEngine) {
        this.embeddingEngine = new EmbeddingEngine(
          this.modelName,
          this.options.maxBatchSize || this._modelInfo.capabilities.maxBatchSize || 8
        );
      }
      ...
      if (!this.engineResourceId) {
        this.engineResourceId = this.resourceManager.registerModel(
  ...
  async embedText(text: string): Promise<EmbeddingResult> {
    ...
    try {
      ...
      if (result.vector.length !== this.dimensions) {
        throw new Error(
          `Embedding dimension mismatch: expected ${this.dimensions}, got ${result.vector.length}`
        );
      }
      ...
      try {
        ...
        for (let i = 0; i < results.length; i++) {
          if (results[i].vector.length !== this.dimensions) {
            throw new Error(
              `Embedding dimension mismatch for item ${i}: expected ${this.dimensions}, got ${results[i].vector.length}`
            );
          }
        }
