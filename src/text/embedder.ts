 ...
 */
export function createTextEmbedFunction(modelName?: string, batchSize?: number): EmbedFunction {
  let engine: EmbeddingEngine | null = null;
  
  const embedFunction: EmbedFunction = async (query: string, contentType?: string): Promise<EmbeddingResult> => {
