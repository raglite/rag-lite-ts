 ...
 */
function detectContentType(content: string, filePath: string): string {
 ...
 */
function needsPreprocessing(content: string, contentType: string): boolean {
...
function processContentByType(
  ...
  if (contentType === 'mdx' || ContentTypeDetector.hasJsxContent(content)) {
    processedContent = processWithPreprocessor(
      processedContent,
      'mdx',
      { mode: resolvedConfig.mdx }
    );
  }
 ...
 */
function processWithPreprocessor(
  content: string,
 ...
 */
function processCodeBlocks(content: string, mode: 'strip' | 'keep' | 'placeholder'): string {
  if (mode === 'keep') {
    return content;
  }
 ...
 */
function handleUnknownContentType(content: string, contentType: string): string {
 ...
 */
function handleProcessingFailure(content: string, contentType: string): string {
 ...
 */
function handleCriticalFailure(content: string): string {
